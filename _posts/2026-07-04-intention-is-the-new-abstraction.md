...
- **One rule:** give the contract, not the implementation. Breaking it now costs immediately, on both classical axes: diluted attention and accidental coupling.
...
An LLM is (probably) the first machine that consumes natural contracts in general. You hand it a sentence — about a schema, a codebase, a refund policy — and it acts on that sentence, with no human in between and no domain built for it in advance. The meaning is inferred by the model, not fixed by a language definition.
...
The new layer has no absolute precision: a natural sentence carries none, so you have to pin the meaning yourself — see [The Real Prompt Engineering]({% post_url 2026-04-03-the-real-prompt-engineering %}). This is the one place the new layer is weaker than every layer below it, and it is where the work moved.
...
Program to an interface, not an implementation. **Use the contract, not the implementation.**
...
Your prompt is a contract, and it can carry either side. 
  - *"Loop over each line, split on the equals sign, handle escaped quotes, then retry the write three times"* is an implementation typed into the window: you have done the model's work and left it transcription. 
  - *"Parse this config file and save it; fail if the disk is full"* is a contract — an outcome, a constraint, and nothing about method. Both may come back as working code. Only the second still says what you wanted after the code changes.
...
The price is specific to agents. A human reads a body, closes the file, and pays in time and cognitive load. An agent reads a body and it stays in the window for the rest of the session — every later decision made with those internals present. A descent is not a peek but a permanent change to what the model reasons from.
...
- Terry Winograd, *SHRDLU*, 1970–72 — English commands executed inside a closed blocks world
- Aditya Desai et al., *Program Synthesis Using Natural Language*, ICSE 2016 — English descriptions compiled to programs in a fixed target DSL, one domain at a time
