...
Space: O(n) - must store all values in result
...
class Solution:
    ...
    def dfs(self, isConnected: List[List[int]], visited: List[int], city: int) -> None:
        visited[city]  = True
...
### BFS
Time Complexity: O(n²) - we might visit each cell in the matrix once
...
class Solution:
    def findCircleNum(self, isConnected: List[List[int]]) -> int:
        ...
        return provinces
    def bfs(self, isConnected:List[List[int]], visited: List[bool], city: int) -> None:
        queue = deque([city])
        visited[city] = True
        ...
        while queue:
            ...
            for i in range(len(isConnected)):
                if isConnected[current][i] == 1 and not visited[i]:
                    queue.append(i)
                    visited[i] = True
